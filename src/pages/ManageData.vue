          ...
          <v-card>
            <v-card-title>
              ...
              <v-flex sm6 md4 lg4 xl3 class="filter-select">
                <v-text-field v-model="searchDep" append-icon="search" label="Search" single-line
                  hide-details></v-text-field>
              </v-flex>
              <v-btn color="#007fc4" dark @click="showDepAdd('add')">ADD</v-btn>
            </v-card-title>
            <v-data-table :headers="headersDep" :search="searchDep" :items="departments">
              <template v-slot:items="props">
                <tr>
                  ...
                  <td class="text-left">
                    <v-icon class="custom-icon" size="18" color="white"
                      @click="showDepAdd('edit', props)">mdi-pencil</v-icon>
                    <v-icon class="custom-icon-delete" size="18" color="white"
                      @click="deleteDep(props.item.departmentID)">mdi-delete</v-icon>
                  </td>
                </tr>
              </template>
            </v-data-table>
          ...
          <v-card>
            <v-card-title>
              ...
              <v-flex md3 xl3 lg3 class="filter-search-position">
                <v-text-field v-model="searchPos" append-icon="search" label="Search" single-line
                  hide-details></v-text-field>
              </v-flex>
              <v-flex md3 xl3 lg3 class="filter-department">
                <v-autocomplete outlined v-model="filterDepartment" :items="['All Department', ...departmentList]" dense
                  label="Filter Department" return-object hide-details></v-autocomplete>
              </v-flex>
              <v-btn color="#007fc4" dark @click="setPositionModel('add')">ADD</v-btn>
            </v-card-title>
            <v-data-table :headers="headersPos" :search="searchPos" :items="positions">
              <template v-slot:items="props">
                <tr>
                  ...
                  <td>
                    <v-icon class="custom-icon" size="18" color="white"
                      @click="setPositionModel('edit', props)">mdi-pencil</v-icon>
                    <v-icon class="custom-icon-delete" size="18" color="white"
                      @click="deletePos(props.item.positionID, props.item.position_Name)">mdi-delete</v-icon>
                  </td>
                </tr>
              </template>
            </v-data-table>
        ...
        <v-form ref="form" style="padding: 20px 30px 0px 30px;">
          <v-layout wrap>
            ...
            <v-flex xl9 lg9 md9 sm8>
              <v-text-field v-model="descEN" label="EN" @keydown.native="keyFilter($event, 'Group')"
                prepend-inner-icon="mdi-office-building-plus"></v-text-field>
            </v-flex>
          </v-layout>
          <v-layout wrap>
            <v-flex xl3 lg3 md3 sm4 xs12 d-flex align-center>
            ...
            <v-flex xl9 lg9 md9 sm8>
              <v-text-field v-model="descTH" label="TH" @keydown.native="keyFilter($event, 'GroupTh')"
                prepend-inner-icon="mdi-office-building-plus"></v-text-field>
            </v-flex>
          </v-layout>
          <v-layout wrap>
            ...
            <v-flex xl9 lg9 md9 sm8>
              <v-select v-model="descStatus" :items="statusList" label="Select Status" prefix="*"
                class="prefix"></v-select>
            </v-flex>
          </v-layout>
        </v-form>
    ...
    <v-dialog v-model="showPosDialog" persistent width="75%">
      <v-card style="padding-bottom: 10px;">
        <v-card-title style="font-weight: bold; display: flex; align-items: center; justify-content: space-between;">
          ...
          <v-icon size="30" color="white" style="display: flex; justify-content: flex-end;"
            @click="showPosDialog = false">mdi-close-box</v-icon>
        </v-card-title>
                ...
                <div v-if="item === 'Position'" style="padding-top: 10px;">
                  <v-layout wrap>
                    ...
                    <v-flex xl10 lg10 md9 sm9>
                      <v-text-field v-model="posName" type="text" label="Position Name (EN)"
                        @keydown.native="keyFilter($event, 'Group')"
                        prepend-inner-icon="mdi-briefcase-plus"></v-text-field>
                    </v-flex>
                  </v-layout>
                  <v-layout wrap>
                    <v-flex xl2 lg2 md3 sm3 xs12 d-flex align-center>
                    ...
                    <v-flex xl10 lg10 md9 sm9>
                      <v-autocomplete v-model="depID" :items="departments" item-text="department_DescEN"
                        item-value="departmentID" label="Select Department"
                        prepend-inner-icon="mdi-office-building"></v-autocomplete>
                    </v-flex>
                  </v-layout>
                  ...
                  <v-layout wrap>
                    ...
                    <v-flex lg4 sm5 xs12>
                      <v-select v-model="posStatus" :items="statusList" label="Select Status" prefix="*"
                        class="prefix"></v-select>
                    </v-flex>
                    ...
                    <v-flex lg4 sm4 xs12>
                      <v-select v-model="posPriority" :items="priorityList" label="Select Priority" prefix="*"
                        class="prefix"></v-select>
                    </v-flex>
                  </v-layout>
                </div>
                ...
                <div v-else-if="item === 'Description'">
                  <v-layout wrap>
                    <v-flex xl6 lg6 md6 sm6 xs12 class="padding-en">
                      ...
                      <v-textarea rows="1" v-model="jobDescEN" type="text" label="EN"
                        @keydown.native="keyFilter($event, 'Group')" background-color="white"></v-textarea>
                    </v-flex>
                    <v-flex xl6 lg6 md6 sm6 xs12 class="padding-th">
                      <label class="custom-label-detail">Description (TH)</label>
                      <v-textarea rows="1" v-model="jobDescTH" type="text" label="TH"
                        @keydown.native="keyFilter($event, 'GroupTh')" background-color="white">
                        <template v-slot:append-outer>
                          <v-icon @click="descItemButton === 'old' ? validateDescItemEdit() : validateDescItemAdd()"
                            :size="35" color="#F9A825">
                  ...
                  <v-data-table :headers="headersDescDetail" :items="descDetailData">
                    <template v-slot:items="props">
                      <tr>
                        ...
                        <td class="text-left" style="width: 10rem;">
                          <v-icon class="custom-icon" size="18" color="white"
                            @click="setItemDescModel(props)">mdi-pencil</v-icon>
                          <v-icon class="custom-icon-delete" size="18" color="white"
                            @click="deleteItemDescription(props.index)">mdi-delete</v-icon>
                        </td>
                      </tr>
                    </template>
                  </v-data-table>
                ...
                <div v-else-if="item === 'Qualification'">
                  <v-layout row wrap>
                    <v-flex xl6 lg6 md6 sm6 xs12>
                      ...
                      <v-textarea rows="1" v-model="jobQuaEN" type="text" label="EN"
                        @keydown.native="keyFilter($event, 'Group')" background-color="white"></v-textarea>
                    </v-flex>
                    <v-flex xl6 lg6 md6 sm6 xs12>
                      <label class="custom-label-detail">Qualification (TH)</label>
                      <v-textarea rows="1" v-model="jobQuaTH" type="text" label="TH"
                        @keydown.native="keyFilter($event, 'GroupTh')" background-color="white">
                        <template v-slot:append-outer>
                          <v-icon @click="quaItemButton === 'old' ? validateQuaItemEdit() : validateQuaItemAdd()"
                            :size="35" color="#F9A825">
                  ...
                  <v-data-table :headers="headersQuaDetail" :items="quaDetailData">
                    <template v-slot:items="props">
                      <tr>
                        ...
                        <td class="text-left" style="width: 10rem;">
                          <v-icon class="custom-icon" size="18" color="white"
                            @click="setItemQuaModel(props)">mdi-pencil</v-icon>
                          <v-icon class="custom-icon-delete" size="18" color="white"
                            @click="deleteItemQualification(props.index)">mdi-delete</v-icon>
                        </td>
                      </tr>
                    </template>
                  </v-data-table>
