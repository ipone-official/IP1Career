    ...
    <v-form ref="form" v-model="valid">
      ...
      <v-dialog v-model="dialog2" width="80%" persistent>
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" @click="dialog2 = false" :disabled="currentPage != 11">ยอมรับ</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
...
.custom-filename-upload {
  ...
  padding: 6px 12px;
  margin-bottom: 15px;
  background-image: linear-gradient(to right, rgba(51, 148, 225, 0.18), transparent);
  width: 100%;
}
